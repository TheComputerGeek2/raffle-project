...
public class InputFilter implements KeyListener {
	...
	public static final String VALID_DECIMAL_INPUTS = Messages.getString("InputFilter.0"); //$NON-NLS-1$
	
	public static final String VALID_HEXIDECIMAL_INPUTS = Messages.getString("InputFilter.1"); //$NON-NLS-1$
	
	public static final int MAX_INPUT_LENGTH = 10;
	
	private JTextField source;
	
	public static enum NumberSystem {DECIMAL, HEX}
 ...
 * @param arg0 the key event to respond to.*/
 ...
 * @param validInputs the characters to allow.
...
private void enforceValidInputCharacters(String input, String validInputs) {
	for (int i = 0; i < input.length(); i++) {
		if (!checkCharacter(input.charAt(i) + Messages.getString("InputFilter.2"), validInputs)) { //$NON-NLS-1$
 ...
 * @param text the character to check in the form of a string.
 * @param validInputs a string containing all of the valid inputs.
 * @return if the input is valid.*/
private boolean checkCharacter(String text, String validInputs) {
	for (int i = 0; i < validInputs.length(); i++) {
		if (text.contains(validInputs.charAt(i) + Messages.getString("InputFilter.3"))) { //$NON-NLS-1$
			return true;
		}
	}
	return false;
