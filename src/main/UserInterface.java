...
public class UserInterface implements KeyListener, MouseListener {
	...
	/**The maximum number of extra rounds beyond the base
	 * rounds.*/
	...
	public static final Color GREEN = new Color(23, 172, 23); // original was 13 172 43
		
	/**The DimensionsProcessor used to determine
	 * what size to make the frame and fonts.*/
	public static final DimensionsProcessor dp = new DimensionsProcessor(BASE_WIDTH, BASE_HEIGHT);
	...
	protected static final Font INPUT_FONT = new Font(Font.SANS_SERIF, Font.BOLD, dp.scaleValue(24));
	
	/**The font used for the output label.*/
	protected static final Font OUTPUT_FONT = new Font(Font.SANS_SERIF, Font.BOLD, dp.scaleValue(50));
	
	/**The object that picks the numbers and displays them.*/
	private Picker np;
	
/**Instantiates a new user interface of the raffle program.
* @throws AWTException an exception thrown by the ThreePanelPicker.
* Or if a toolkit could not be found, or if one could
* not be accessed or instantiated.*/
 ...
 * or if one could not be accessed or instantiated.*/
private Point centerFrame(Container frame) {
...
private Container makeFrame(Container frame) {
	frame.setSize(new Dimension(dp.scaleValue(BASE_WIDTH), dp.scaleValue(BASE_HEIGHT)));
 ...
 * @param arg0 the source of the event.*/
private void response(Object arg0, Integer keyCode) {
	if (arg0.equals(pickNumber)) {
		if (EventFilter.buttonEventFilter(keyCode)) {
			pickingSequence();
		}
	} else if (KeyEvent.getKeyText(keyCode).equalsIgnoreCase(Messages.getString("UserInterface.4"))) { //$NON-NLS-1$
		shiftFocus(arg0);
	}
}

/**Shifts the focus from one component to another.
 * @param arg0 the source of the event that
 * triggered the response method.*/
...
private void pickingSequence() {
	getInputs();
		if (getInputs() &&verifyValidInputType() && verifyValidRange() && verifyValidValues()) {
 ...
 * @param out1 the top output component.
 * @param out2 the middle output component.
 * @param out3 the bottom output component.
 * @param value the value to introduce to the system.*/
...
private boolean verifyValidInputType() {
	...
	if (scanner1.hasNextInt() && scanner2.hasNextInt()) {
		if (scanner1.nextLong() > Integer.MAX_VALUE || scanner2.nextLong() > Integer.MAX_VALUE) {
			scanner1.close();
			scanner2.close();
			return false;
		}
 ...
 * @throws NumberFormatError if the inputs does not contain a parsable long.*/
...
public void mouseClicked(MouseEvent arg0) {
	...
	pickingSequence();
	/*if (arg0.getSource().equals(pickNumber)) {
		pickingSequence();
	} else {
